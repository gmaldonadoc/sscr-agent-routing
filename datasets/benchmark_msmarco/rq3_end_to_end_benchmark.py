...
Then run a small smoke test without API calls:

    python datasets/benchmark_msmarco/rq3_end_to_end_benchmark.py \
        --skip-llm --max-queries 5 --n-values 100,500 --k-values 5,10

Finally run the paper experiment (requires collection.tsv and OPENAI_API_KEY):

    python datasets/benchmark_msmarco/rq3_end_to_end_benchmark.py \
        --n-values 100,500,1000,5000 --k-values 5,10,20,50
"""
...
DEFAULT_OUTPUT_DIR = BASE_DIR / "results" / "rq3_corrected"
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    for query_index, item in enumerate(selected_queries, start=1):
        ...
        for n in args.n_values:
            ...
            # Offline index construction is performed once per registry and is
            # deliberately excluded from online end-to-end latency.
            registry_index, offline_latency = build_ssr_registry_index(
                candidates=full_catalog,
                indexer=indexer,
            )

            ranked, retrieval_latency, query_signal_count = retrieve_ssr_ranked(
                query=query,
            ...
            for k in args.k_values:
                ssr_candidates = ranked[:k]
                ssr_candidate_ids = {doc["pid"] for doc in ssr_candidates}
                retrieval_hit = int(bool(positive_ids.intersection(ssr_candidate_ids)))

                # No full-catalog fallback. An empty result is an observable
                # retrieval failure and no LLM call is made for that row.
                llm_result, _ = evaluate_llm_pool(
                    query=query,
                ...
                rows.append(
                    make_result_row(
                        ...
                        offline_index_latency_s=offline_latency,
    ...
    metadata = {
        ...
        "metrics": {
            "candidate_recall": "relevant passage appears in the SSR top-K pool",
            ...
            "online_end_to_end_latency": (
                "online SSR retrieval + successful LLM inference; offline index construction excluded"
            ),
        },
